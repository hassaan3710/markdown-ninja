        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="min-w-full bg-white divide-y divide-gray-200">
            <tr v-for="invitation in invitations" :key="invitation.id" class="table-row min-w-full">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <sl-button variant="neutral" circle @click="$emit('accept', invitation)">
                  <CheckIcon class="h-5 w-5" aria-hidden="true" />
                </sl-button>
                <sl-button variant="neutral" circle @click="$emit('decline', invitation)">
...
defineProps({
  invitations: {
    type: Array as PropType<UserInvitation[]>,
    required: true,
  },
});
