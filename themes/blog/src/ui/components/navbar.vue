<template>
  <!-- <nav> -->
    <Disclosure as="nav" class="w-full shadow-sm bg-[var(--mdninja-background)]" v-slot="{ open }">
      <div class="mx-auto max-w-7xl px-2 xs:px-4">
      <div class="flex h-16 justify-between w-full">
        ...
        <div class="flex items-center content-center space-x-3 sm:space-x-4">
          ...
          <PLink v-if="subscribeButton && !$store.contact" href="/login" class="group relative hidden sm:inline-flex hover:no-underline">
            <button type="button"
              class="p-2.5 inline-flex rounded-full bg-[var(--mdninja-background)] text-gray-500 shadow-none hover:bg-[var(--mdninja-background)] hover:brightness-[0.95]">
              <UserIcon class="size-5" aria-hidden="true" />
            </button>

            <!-- <span class="absolute hidden group-hover:flex -top-2 -ml-7 translate-y-full w-16 text-center mt-7 px-2 py-1 bg-[var(--mdninja-accent)] rounded-lg text-center text-white text-sm after:content-[''] after:absolute after:left-1/2 after:bottom-[100%] after:translate-x after:border-8 after:border-x-transparent after:border-t-transparent after:border-b-(--mdninja-accent)">
                Sign in</span> -->
          </PLink>
...
import { Bars3Icon, XMarkIcon, EllipsisVerticalIcon, MagnifyingGlassIcon, ChevronDownIcon, UserIcon } from '@heroicons/vue/24/outline'
